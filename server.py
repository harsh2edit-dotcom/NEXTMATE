...
Runs the webhook, the device-verification website, and the admin dashboard.
All HTML/CSS/JS is inline (no separate template / static files).
"""
import os
import uuid
import asyncio
...
SESSION_SECRET = os.environ.get("SESSION_SECRET", "xyven-session")
WEBHOOK_SECRET = os.environ.get("WEBHOOK_SECRET", "xyven-hook")
...
# Global bot/dispatcher — initialised in lifespan
...
async def lifespan(app: FastAPI):
    ...
    if base:
        ...
        b.log.info("Webhook set: %s/webhook/***", base)
    else:
        b.log.warning("WEBHOOK_BASE_URL not set — webhook disabled (use polling for local dev)")
...
# ════════════════════════════════════════════════════════════════════
# VERIFICATION WEBSITE  (HTML + CSS + JS all inline)
# ════════════════════════════════════════════════════════════════════
...
if(token){ document.getElementById('go').textContent = '🔐 I Agree — Start Verification'; }
</script></body></html>
"""
...
# ════════════════════════════════════════════════════════════════════
# ADMIN DASHBOARD  (HTML + CSS + JS all inline)
# ════════════════════════════════════════════════════════════════════
